...
def main():
    ...
    img_gray = cv2.imread("/Users/akhi/Desktop/DIP/images/tulip.png", 0)
    ...
    custom_1 = np.array([[ 0, -1,  0],
                         [-1,  5, -1],
                         [ 0, -1,  0]], dtype=np.float32)

    # Emboss kernel
    custom_2 = np.array([[-2, -1,  0],
                         [-1,  1,  1],
                         [ 0,  1,  2]], dtype=np.float32)

    # Diagonal edge detection
    custom_3 = np.array([[ 1,  1,  0],
                         [ 1,  0, -1],
                         [ 0, -1, -1]], dtype=np.float32)

    # Sharpening kernel
    custom_4 = np.array([[ 0, -1,  0],
                         [-1,  9, -1],
                         [ 0, -1,  0]], dtype=np.float32)
    ...
    filters = {
        ...
        "Scharr-X": scharr_x,
        "Scharr-Y": scharr_y,
        "Custom-EdgeEnhance": custom_1,
        "Custom-Emboss": custom_2,
        "Custom-DiagonalEdge": custom_3,
        "Custom-Sharpen": custom_4
    }
